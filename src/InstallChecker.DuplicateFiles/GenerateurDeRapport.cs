    ...
    public static RapportDeDoublons Generer(W w, IObservationsSource omega)
    {
        ...
        foreach (var groupeActeW in groupesActeW)
        {
            ...
            var exemplaires = exemplairesClasses
                .Select(e => new ExemplaireRapporte(e.Fichier, e.Rang, Etiquette(e.Rang), e.Motif))
                .ToList();

            groupes.Add(new GroupeClasse(
                groupeActeW.Domaine, MotifCourt(groupeActeW), tailleUnitaire, espaceRecuperable, exemplaires));
        }
        ...
        return new RapportDeDoublons(synthese, note, groupes);
    }
