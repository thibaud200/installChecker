...
public static class ScanCommand
{
        ...
        string? jsonFilePath = null)
    {
        if (jsonOutput && jsonFilePath is not null)
            throw new ArgumentException("La sortie JSON standard et le fichier JSON sont mutuellement exclusifs.");
        ...
        using (store)
        {
            ...
            foreach (var file in new DirectoryInfo(root).EnumerateFiles("*", options))
            {
                ...
                try
                {
                    ...
                    var versionInfo = VersionInfoObservation.From(FileVersionInfo.GetVersionInfo(file.FullName));
                    ...
                    if (!jsonOutput && jsonFilePath is null)
            ...
            if (jsonOutput || jsonFilePath is not null)
            {
                try
                {
                    ...
                    {
                        using var fichier = new StreamWriter(jsonFilePath, append: false, new UTF8Encoding(false));
                        foreach (var ligne in store.ProjeterJsonDuScan())
                ...
                {
                    errors.WriteLine($"Erreur : export JSON impossible : {jsonFilePath} : {ex.Message}");
                    return 1;
                }
            }
