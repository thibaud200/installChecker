# AGENTS.md — Contrat d’architecture et règles du projet
...
Développer une plateforme modulaire construite autour d'un moteur d'identité logique générique.

Le dépôt est organisé en deux niveaux :

- un cœur générique, réutilisable indépendamment de tout domaine métier ;
- un ou plusieurs modules métier illustrant son utilisation.

Le moteur ne connaît ni le domaine métier, ni les formats analysés. Il ne manipule que des observations Ω et un registre de conventions R afin de dériver un état du monde W.

La théorie du moteur est définie exclusivement dans `docs/identity/000` à `docs/identity/026`.
...
Le dépôt a vocation à servir de plateforme pour plusieurs modules métier indépendants partageant le même moteur d'identité.

Chaque module métier :

- définit ses propres observations Ω ;
- fournit son propre registre de conventions R ;
- exploite le même moteur d'identité sans modification de celui-ci.

Le dépôt peut accueillir plusieurs modules métier indépendants partageant le même moteur.
Le premier module développé est actuellement décrit dans `docs/modules/vision.md`.

---

## Principe de modularité

Aucun besoin métier ne doit conduire à modifier le moteur d'identité lorsque ce besoin peut être satisfait par un module.

Les adaptations métier sont réalisées exclusivement :

- par les observations Ω ;
- par le registre ℛ ;
- par les modules utilisant le moteur.

Le moteur n'évolue que pour répondre à un besoin générique applicable à tous les domaines.
...
## ADR-012 — Schéma v3 : snapshots immuables mutualisés et occurrences append-only

- **Contexte** : le schéma v2 préserve l'historique mais recopie toutes les observations brutes et leurs capacités à chaque rescan, même lorsqu'elles sont strictement identiques. Identity et Identity.Access sont gelés.
- **Décision** : le Scanner écrit `user_version = 3`. Les valeurs brutes lourdes sont stockées une seule fois dans un snapshot immuable identifié par un encodage canonique versionné de toutes les observations. Chaque chemin observé reste une occurrence append-only liée à son scan et à ce snapshot. Un adaptateur appartenant au module Scanner projette chaque occurrence courante comme un acte Omega distinct et délègue les bases v1/v2 au lecteur historique gelé.
- **Alternatives** : conserver la copie intégrale v2 (coût de stockage) ; upsert par chemin (historique perdu) ; déduplication par seul hash (métadonnées et contrat d'extraction ignorés) ; modifier Identity.Access (gel architectural violé).
- **Conséquences** : ADR-002 est raffiné, pas supprimé : l'append-only s'applique aux scans et occurrences, tandis que des payloads immuables strictement identiques peuvent être mutualisés. Deux copies à deux chemins restent deux actes et donc deux exemplaires pour Duplicate Files. Les nouvelles bases sont v3, sans migration automatique ; v1/v2 restent lisibles. Le JSON n'est émis qu'après commit et `--json-file` remplace le fichier existant.
...
- Phase 6 : premier module métier « Duplicate Files » — **en cours** (rapport de doublons `duplicates`, plan de suppression `plan`, politique de rétention versionnée, sélection de corpus `--ext`, gestion multi-disque et stockage Scanner v3 dédupliqué, état courant par volume, volumes dans le rapport)
...
Le présent `AGENTS.md` décrit uniquement :

- la vision générale du dépôt ;
- l'architecture globale ;
- les décisions d'architecture (ADR) ;
- les contraintes de développement ;

sans dupliquer les spécifications théoriques du moteur ni la vision fonctionnelle du projet.

## Consignations

Consignations de conformité (jamais normatives) :

- `docs/conformite/`
  - déclarations de conformité v1, v2 et v3 ;
  - les trois écarts publiés ;
  - la forme canonique matérielle de W et de τ.
